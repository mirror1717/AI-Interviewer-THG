...
class DiHumanProcessor:
    def __init__(self, data_path):
        ...
        # 只保留 .jpg / .lms，并按编号排序更稳妥
        img_files = [f for f in img_files if f.endswith(".jpg")]
        lms_files = [f for f in lms_files if f.endswith(".lms")]
    ...
    def process(self, audio_frame):
        ...
        if self.audio_queue_get_feat.shape[0]>=11040:  # 攒够690 ms的音频后开始处理
            ...
            if self.using_feat.shape[0]>=16: # 音频特征攒够8帧 开始输出图片 下面的逻辑和inference里的一样
                ...
                audio_feat = self.using_feat.reshape(1,256,16,32)
